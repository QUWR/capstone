...
* 사용자 정보(이름, 이메일, 비밀번호, 나이, 키, 몸무게)와 함께 식습관(Habit) 및 선호도(Preference)를 입력받아 맞춤형 서비스의 기반을 마련합니다.
* Spring Security와 JWT를 활용하여 안전한 인증 및 인가 시스템을 구현했습니다.
...
* **레시피 데이터 관리**: 생성된 레시피는 이름, 설명, 재료 목록(이름, 수량), 단계별 조리법(설명, 분/초 단위 조리 시간)으로 구조화되어 데이터베이스에 저장됩니다.

### 3. 스마트 대체 재료 시스템 (LLM 기반)
...
- **자동 영양 성분 분석:** 레시피가 생성되거나 재료가 변경될 때마다 Flask API를 호출하여 칼로리, 탄단지 등 9가지 영양 성분(칼로리, 탄수화물, 단백질, 지방, 당, 나트륨, 포화지방, 트랜스지방, 콜레스테롤)을 실시간으로 계산하고 독립적인 트랜잭션으로 저장합니다.

### 5. 사용자 만족도 평가

* 사용자는 제공받은 레시피에 대해 평점(rate)과 코멘트(comment)를 남겨 피드백을 제공할 수 있습니다.
* 이러한 피드백 데이터는 추후 AI 모델 개선 및 맞춤형 추천 고도화에 활용될 수 있습니다.
...
| **GET** | `/api/recipe/{recipeId}/refresh-nutrition` | 레시피 영양 정보 갱신 |
| **POST** | `/api/recipe/{id}/satisfaction` | 레시피 만족도 및 피드백 저장 |
| **POST** | `/api/chat` | 사용자 정보(User, Recipes, Satisfactions)를 Flask 서버로 전송 |

## 📊 데이터 구조 (Entity)

- **User:** 사용자 기본 정보, 신체 정보 및 식습관/선호도 관리
- **Recipe:** 레시피 기본 정보(이름, 설명) 및 사용자 매핑
- **Ingredient:** 상세 재료 (이름, 수량)
- **Instruction:** 단계별 조리법 (내용, 분/초 단위 시간 포함)
- **Nutrition:** 레시피별 상세 영양 성분 데이터 (칼로리, 탄수화물, 단백질 등 9종)
- **Satisfaction:** 사용자의 레시피 평가 데이터 (평점, 코멘트)
